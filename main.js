...
ipcMain.handle('stats:export-md', async (event, { timeRange, selectedDate, customStart, customEnd, timeLabel }) => {
  ...
  md += '**卡手与废件分析（含构筑背景）**：\n';
  md += '- 卡手标记中的"卡废件"（cantPlayGarnet）指不希望开局被自然抽到、应由卡组效果调度、抽到后难以处理的牌。废件在构筑中的特征：同名卡只带1张。\n';
  md += '- 分析"卡废件"时需注意：\n';
  md += '  - 不能仅看废件总次数（如203次），还需结合构筑中废件种类数量来判断。\n';
  md += '  - 优质动点通常带3张，一般动点和补点带2~3张，大解牌（后手为主）带1~2张，后手手坑带满3张，只有特定废件类卡是同名只带1张。\n';
  md += '  - 如果卡组中有3~5种废件（各1张），在先手5张中至少抽到1张废件的理论概率明显高于单张废件的2.5%起手率——因此"卡废件"次数高更可能是废件种类多的结果，不一定是单卡概率异常。\n';
  md += '  - 社区存在体感：只带1张的牌上手率高得不正常。分析时应提及此体感，但说明现有数据无法区分是废件种类过多还是确实存在单卡层面的随机偏差，需要完整的构筑列表数据才能验证。\n\n';
  ...
  md += '- **废件上手率**：社区体感只带1张的废件上手率高得不正常。分析时应从构筑角度说明：废件同名卡只带1张，若卡组中有N种废件，合计起手率远高于2.5%。在缺乏具体构筑数据的情况下，将"卡废件"次数与对应卡组的废件种类数做交叉分析（如柜奏系废件种类 vs 其他卡组废件种类），若各卡组废件率差异可被废件种类数解释，则不支持异常。若废件率差异与种类数无关，则需标记为嫌疑。\n\n';
  md += '### 3. 注意事项\n\n';
