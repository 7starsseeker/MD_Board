...
ipcMain.handle('stats:export-md', async (event, { timeRange, selectedDate, customStart, customEnd, timeLabel }) => {
  ...
  md += '- **投币总体公平**：统计全量投币正率是否接近 50%，Z 检验分数是否正常\n';
  md += '- **升降段投币倾斜**：晋级赛 vs 保级赛的投币正率差距，差距越大嫌疑越高\n';
  md += '- **连败后补偿**：连败 ≥4 后下一局的胜率、投币正率、卡手率是否异常向好的方向偏\n';
  md += '- **连胜后惩罚**：连胜 ≥4 后下一局的胜率、投币正率、卡手率是否异常向差的方向偏\n';
  md += '- **后手连压模式**：连反 ≥4 的频率和时长、孤立先手占比是否超出正常随机分布\n';
  md += '- **手牌质量操控**：保级赛的卡手率是否显著高于晋级赛（若无差异则无证据）\n';
  md += '- **对手强度调节**：连败后对手强度是否明显下降、连胜后对手强度是否明显上升\n\n';
  md += '### 3. 注意事项\n\n';
