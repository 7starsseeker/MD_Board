...
ipcMain.handle('stats:export-md', async (event, { timeRange, selectedDate, customStart, customEnd, timeLabel }) => {
  ...
  md += '**硬币序列分析**：\n';
  md += '- 统计先手（正）之间的后手间隔数分布，识别"孤立先手"（反→正→反）和"后手连压"（连反≥4）模式\n';
  md += '- 条件概率转移矩阵：前1局/前2局硬币结果对下一局的条件概率（如正→正、反→反→正等），判断是否存在短期非随机记忆\n';
  md += '- 小样本偏移评估：样本量较少时（N<200），使用二项分布 Clopper-Pearson 精确置信区间（95%置信水平）判断观察到的正率偏移是否仍属于正常随机波动\n';
  md += '- 连败区间孤立先手审查：连败（连反≥3）中出现的孤立先手局（反→正→反），逐局检查其卡手率、手坑种类/数量、对手卡手情况和终场强度\n';
  md += '- 连胜区间孤立后手审查：同理分析连胜中出现的孤立后手局的对局质量\n\n';
  md += '**时段分析**：\n';
  ...
  md += '- 不同日：按具体日期分组统计，识别单日异常波动\n';
  md += '- 跨时段机制连续性：连胜连败补偿、硬币偏差等机制在不同月份/不同周之间是否持续一致，段位重置后机制触发条件是否归零重置\n\n';
  ...
  md += '- **后手连压模式**：连反≥4频率、时长、孤立先手占比是否超出随机\n';
